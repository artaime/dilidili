...
flowchart LR
  ...
  OCSession --> Plugin[dili 插件]
  Plugin --> ChannelCmd[角色配置命令<br/>openclaw config set channels.dili ...]
...
3. 在 OpenClaw 控制台角色配置中依次执行以下四条命令：
   `openclaw config set channels.dili.enabled true --strict-json`
   `openclaw config set channels.dili.url "{url}"`
   `openclaw config set channels.dili.token "{token}"`
...
3. 安装和配置完成后，即可在 OpenClaw 会话中调用 dili 插件能力。
4. 在 `查看openclaw` 弹层可使用“发送测试”验证连通性与回复。
5. 在设备侧可通过 `打开龙虾` / `进入龙虾` 进入 OpenClaw 模式，通过 `关闭龙虾` / `退出龙虾` 退出模式。

## 排查建议

- 状态显示未连接：确认 `channels.dili.url` 与 `channels.dili.token` 使用的是最新值，且 `channels.dili.enabled` 已设为 `true`。
