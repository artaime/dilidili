...
> 声纹识别（Speaker Identification）是 dili-esp32-server-golang 项目中的一项核心功能，用于识别设备端用户的身份，并根据识别结果动态切换 TTS 音色。
...
```
┌──────────────────┐     ┌──────────────────────┐     ┌──────────────────┐
│   ESP32 设备     │────▶│ dili-esp32-server │────▶│   voice-server   │
│  (采集音频)      │     │     (主服务)          │     │ (声纹识别服务)   │
└──────────────────┘     └──────────────────────┘     └──────────────────┘
                                                              │
                                                              ▼
...
| 组件 | 职责 |
|------|------|
| **dili-esp32-server** | 主服务，负责设备连接、会话管理、声纹识别结果处理 |
