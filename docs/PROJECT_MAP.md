...
| 对话/留言隐私加密 | 落盘 AES-GCM；`PRIVACY_KEK_BASE64`；表 `device_encryption_keys`；管理端仅本人绑定设备可查看隐私 | `manager/backend/privacy/`；见 `docs/dev/FEATURE_privacy_encryption.md`、`docs/features/admin-device-privacy-gate/FEATURE.md` |
| 管理端设备列表 | `GET /api/admin/devices` 分页筛选 + `agent_stats`；`GET /api/admin/devices/:id` | `DeviceService.ListPaged`；见 `docs/features/admin-device-list-filter/FEATURE.md` |
