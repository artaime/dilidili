...
| `msg_play` | 「播放留言」「继续播放」「再播一遍」 | 播待播 / 重播上一条（不改 DB 状态） |
...
1. **首条或与前一条间隔 >3 小时**：TTS 询问是否收听
2. **肯定** → 播放 → `played`
3. **否定** → 友好结束，**状态保持** `pending`/`notified`（不支持跳过）
4. 同批次间隔 ≤3 小时：过渡语后直接播放
5. 用户说「播放留言」：`msg_play`，跳过询问直接播
...
使用中通过 `chat.parent_message.poll_interval_sec`（默认 30s）轮询；检测到**新留言 ID**（相对上次快照）且 hello+UDP 就绪时，再次主动询问。用户拒绝后同一条留言不会重复询问，直至家长发送新留言。
