...
func initDeviceLog() {
	path := viper.GetString("log.device_path")
	if path == "" {
		path = log.ResolveDeviceLogPath()
	} else if !filepath.IsAbs(path) {
		if root := log.FindRepoRootForLog(); root != "" {
			path = filepath.Join(root, path)
		} else {
			binPath, _ := os.Executable()
			path = filepath.Join(filepath.Dir(binPath), path)
		}
	}
	if err := log.InitDeviceLog(path); err != nil {
		fmt.Printf("init device log warning: %v\n", err)
	}
}
