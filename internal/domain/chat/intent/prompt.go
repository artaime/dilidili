...
func BuildClassifierSystemPrompt(agentSystemPrompt string) string {
	...
	b.WriteString(`你是儿童语音助手的意图路由器。根据「近期对话」与「当前用户说」判断意图，只输出一行 JSON，不要 markdown，不要解释。

必须结合近期对话消解指代（第几个、那个、刚才说的、我刚刚问的等）。看不清或需要回忆上一轮聊天内容时，设 needs_dialogue=true。
...
- device：询问或调节本机设备状态/固件能力。例：「音量多少」「电量多少」「还有多少电」「亮度调到80」「大声一点」「小声一点」「音量设置到50」「去睡觉」「关机」。data 用 {}。本意图由后续主对话处理，勿编造设备状态。
- general：其它闲聊、指代消解、对话回忆、事实介绍等，**一律交主对话处理**。可不填 data.reply。对话回忆（「我刚刚问的是什么」）不是留言；承接上文介绍第几个/真实事物，不是留言能力列表。

约束：
- 不要把指代短句（介绍第二个、那个是什么）判成留言，除非近期对话明确在聊留言。
- 不确定就 needs_dialogue=true，intent 可用 general。

输出格式：
{"intent":"msg_inquiry|msg_play|device|general","confidence":"0.95","needs_dialogue":false,"data":{...}}`)
	return b.String()
}
