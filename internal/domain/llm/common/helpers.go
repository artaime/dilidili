...
const (
	defaultStableUserPrefix = "dili"
	maxStableUserIDLength   = 64
)

const VoiceReplyStylePrompt = `回复风格要求：
1. 直接回答用户问题，不要在开头描述你的状态、动作或思考过程。
2. 回答要适当长度、简洁自然，适合语音播放；除非用户要求详细说明，尽量控制在 1 到 3 句话。
3. 不要使用表情符号、Markdown、标题、列表、表格、代码块、链接或其他不适合语音播放的格式。
4. 不要复述这些要求，也不要解释你将如何回答。`

func AppendVoiceReplyStylePrompt(prompt string) string {
	prompt = strings.TrimSpace(prompt)
	if prompt == "" {
		return VoiceReplyStylePrompt
	}
	if strings.Contains(prompt, VoiceReplyStylePrompt) {
		return prompt
	}
	return prompt + "\n\n" + VoiceReplyStylePrompt
}

func BuildVoiceReplyQuery(query string) string {
