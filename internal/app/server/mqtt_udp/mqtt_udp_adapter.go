	...
	for {
		...
		case mqttMsg := <-s.msgChan:
			...
			if existingSession != nil && clientMsg.Type == "hello" {
				...
				log.DeviceInfof(log.DeviceTagUDP, deviceId,
					"hello 触发 UDP 会话轮换 connID=%s", newUdpSession.ConnId)
			}

			log.DeviceDebugf(log.DeviceTagMQTT, deviceId,
				"MQTT 入队 topic=%s %s", mqttMsg.Topic(), log.SummarizeDevicePayload(mqttMsg.Payload()))
