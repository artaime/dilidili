...
func TestGetMessagesAppendsVoiceReplyStylePrompt(t *testing.T) {
	manager := newTestLLMManager(data_client.MemoryModeShort)
	messages := manager.GetMessages(context.Background(), schema.UserMessage("今天吃什么？"), 10, nil)

	if len(messages) == 0 || messages[0].Role != schema.System {
		t.Fatalf("expected first message to be system prompt, got %+v", messages)
	}
	systemPrompt := messages[0].Content
	if !strings.Contains(systemPrompt, llm_common.VoiceReplyStylePrompt) {
		t.Fatalf("expected voice reply style prompt to be appended, got %q", systemPrompt)
	}
