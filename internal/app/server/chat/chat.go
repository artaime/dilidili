...
func (c *ChatManager) cmdMessageLoop(ctx context.Context) {
	...
	for {
		...
		log.DeviceInfof(log.DeviceTagProto, c.DeviceID, "上行 MQTT %s", log.SummarizeDevicePayload(message))
		if err := c.handleTextMessage(message); err != nil {
			log.Errorf("处理文本消息失败: %v, 消息内容: %s", err, string(message))
			log.DeviceWarnf(log.DeviceTagProto, c.DeviceID, "处理失败: %v payload=%s", err, log.SummarizeDevicePayload(message))
		}
	}
}
	...
	for {
		...
		if c.hasPendingSpeakRequest() {
			...
			if c.clientState != nil {
				c.clientState.NoteUplinkActivity(time.Now())
			}
...
func (c *ChatManager) handleTextMessage(message []byte) error {
	...
	case "data_insight":
		log.DeviceDebugf(log.DeviceTagProto, c.DeviceID, "遥测 data_insight（已忽略）")
		return nil
	default:
...
func (c *ChatManager) HandleHelloMessage(msg *ClientMessage) error {
	log.DeviceInfof(log.DeviceTagProto, c.DeviceID,
		"收到 hello transport=%s version=%d", strings.TrimSpace(msg.Transport), msg.Version)
	if msg.AudioParams == nil {
		log.DeviceErrorf(log.DeviceTagProto, c.DeviceID, "hello 缺少 audio_params")
	...
	if isDuplicateMqttHello {
		log.Infof("设备 %s 收到 duplicate_hello，仅刷新 UDP/hello 响应（不拆除会话）", clientState.DeviceID)
		log.DeviceInfof(log.DeviceTagProto, c.DeviceID, "duplicate_hello：刷新 UDP 参数，保留会话")
	}

	// 先尽快下发 hello（含 UDP 密钥），避免设备在 Session creating 阶段超时并提前发 UDP
	if err := c.sendHelloResponse(msg); err != nil {
		log.DeviceErrorf(log.DeviceTagProto, c.DeviceID, "hello 响应发送失败: %v", err)
		if isFirstHello || requiresFreshHello {
	...
	log.DeviceInfof(log.DeviceTagProto, c.DeviceID,
		"hello 响应已发送 session_id=%s udp=%s:%d frame_duration=%d",
		clientState.SessionID, viper.GetString("udp.external_host"), viper.GetInt("udp.external_port"),
		clientState.InputAudioFormat.FrameDuration)
	...
	if err != nil {
		...
		log.Warnf("设备 %s hello 后创建 ChatSession 失败: %v", clientState.DeviceID, err)
		return err
	}
	_ = chatSession
...
func (c *ChatManager) sendHelloResponse(msg *ClientMessage) error {
	...
	// 回复设备侧协商的音频参数（如 frame_duration=40），勿用 TTS 下行默认 60ms
	helloAudio := c.clientState.InputAudioFormat
	switch transportType {
	case types_conn.TransportTypeWebsocket:
		return c.serverTransport.SendHello(types_conn.TransportTypeWebsocket, &helloAudio, nil)
	case types_conn.TransportTypeMqttUdp:
		udpConfig, err := c.buildMqttHelloUdpConfig()
		if err != nil {
			return err
		}
		return c.serverTransport.SendHello(types_conn.TransportTypeMqttUdp, &helloAudio, udpConfig)
	default:
		return fmt.Errorf("不支持的传输类型: %s", transportType)
	}
}
...
func (c *ChatManager) HandleMcpMessage(msg *ClientMessage) error {
	log.DeviceInfof(log.DeviceTagMCP, c.DeviceID, "MCP 消息 %s", log.SummarizeDevicePayload(msg.PayLoad))
...
func (c *ChatManager) resetMcpRuntimeOnMqttTransportReady() {
	...
	log.DeviceInfof(log.DeviceTagMQTT, c.DeviceID, "MQTT transport_ready：重置 hello 状态，等待设备 hello")
