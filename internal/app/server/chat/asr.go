...
const defaultFirstSpeechPreAudioMs = 800
				...
				if !skipVad && needVad {
					...
					if state.AsrAudioBuffer.GetAsrDataSize() >= vadNeedMinSize {
						...
						if haveVoice && !clientHaveVoice {
							// 首次检测到语音时，保留一小段预滚音频，避免 VAD 命中偏晚导致句首被截断。
...
func trimFirstSpeechAudio(allData []float32, currentFrameSamples, sampleRate, channels int) []float32 {
	...
	maxPreSpeechSamples := sampleRate * channels * getFirstSpeechPreAudioMs() / 1000
...
func getFirstSpeechPreAudioMs() int {
	if !viper.IsSet("chat.first_speech_pre_audio_ms") {
		return defaultFirstSpeechPreAudioMs
	}
	preAudioMs := viper.GetInt("chat.first_speech_pre_audio_ms")
	if preAudioMs <= 0 {
		return defaultFirstSpeechPreAudioMs
	}
	return preAudioMs
