	...
	api := r.Group("/api")
	{
		...
		auth.Use(middleware.JWTAuth())
		{
			...
			mp := auth.Group("/mp")
			{
				...
				mp.POST("/devices/join", mpDeviceController.JoinDevice)
				mp.GET("/devices", mpDeviceController.ListDevices)
				mp.PATCH("/devices/:id", mpDeviceController.UpdateDevice)
				mp.DELETE("/devices/:id", mpDeviceController.UnbindDevice)
				mp.POST("/devices/:id/invites", mpDeviceController.CreateInvite)
				mp.GET("/devices/:id/members", mpDeviceController.ListMembers)
				mp.DELETE("/devices/:id/members/:userId", mpDeviceController.RemoveMember)
				mp.POST("/devices/:id/leave", mpDeviceController.LeaveDevice)
